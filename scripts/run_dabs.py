...
from dabs.src.datasets.catalog import DATASET_DICT,MULTILABEL_DATASETS, PRETRAINING_DATASETS, UNLABELED_DATASETS
...
def run(args):
    ...
    if config.data_params.dataset not in UNLABELED_DATASETS:
        ssl_online_evaluator = online_evaluator.SSLOnlineEvaluator(
            dataset=config.data_params.dataset,
            z_dim=256,#config.model.kwargs.dim,
            num_classes=system.dataset.num_classes(),
            multi_label=(config.data_params.dataset in MULTILABEL_DATASETS),
        )
        callbacks.append(ssl_online_evaluator)
