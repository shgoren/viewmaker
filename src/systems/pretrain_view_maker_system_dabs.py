...
class PretrainViewMakerSystemDABS(pl.LightningModule):
    ...
    def setup(self, stage):
        ...
        if self.config.model_params.pretrained or False:
    ...
    def wandb_logging(self, emb_dict):
        ...
        if self.global_step % logging_steps == 0:
            ...
            grid = make_grid(torch.cat([img[:amount_images], unnormalized_view1[:amount_images], unnormalized_view2[:amount_images],
                             diff_heatmap, diff_heatmap2, (diff_heatmap-diff_heatmap2).abs()]), nrow=amount_images)
